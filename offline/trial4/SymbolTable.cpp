...
class SymbolInfo
{
    ...
    void setParamName(string param_name){
        this->param_name.push_back(param_name);
    }

    string getParamName(int i){
        return this->param_name.at(i);
    }
        ...
        for(int i = 0; i < n; i++){
            if(symbolinfo[i] != nullptr){
                ...
                while(symbol != nullptr){
                    logout << "  < " << symbol->getName() << " , " << symbol->getType()  << " , " << symbol->getWhichVariable() << " >";
