...
void optimizing(){
    ...
    string firstword;
    string secondword;
    string final_first_w;
    string final_second_w;

    while(getline(readcode,line1)){
        vector<string> first_token;
        vector<string> second_token;
        
        stringstream check_first(line1);
        while(getline(check_first,firstword,' ')){
            stringstream check_first_word(firstword);
            while(getline(check_first_word,final_first_w,',')){
                first_token.push_back(final_first_w);
            }
        }
        writeoptimal(line1);
        if(first_token.size()==3 && first_token.at(0)=="\tmov"){
            getline(readcode,line2);
            stringstream check_second(line2);
            while(getline(check_second,secondword,' ')){
                stringstream check_second_word(secondword);
                while(getline(check_second_word,final_second_w,',')){
                    second_token.push_back(final_second_w);
                }
            }

            if(second_token.size()==3&&second_token.at(0)=="\tmov"){
                int m1=first_token.at(1).compare(second_token.at(2));
                int m2=first_token.at(2).compare(second_token.at(1));
                if(m2==0 && m1==0){
                    //writeoptimal("");
                }
                else{
...
{
       0,   206,   206,   292,   321,   339,   352,   365,   384,   448,
     479,   592,   479,   646,   674,   646,   720,   735,   747,   755,
     762,   771,   786,   808,   818,   843,   850,   857,   869,   901,
     934,   957,   983,   994,  1006,  1006,  1006,  1006,  1006,  1006,
    1006,  1006,  1009,  1025,  1051,  1064,  1074,  1074,  1074,  1089,
    1137,  1171,  1218,  1255,  1283,  1307,  1314,  1328,  1335,  1372,
    1414,  1425,  1503,  1514,  1562,  1574,  1585,  1635,  1646,  1679,
    1690,  1762,  1788,  1802,  1815,  1827,  1949,  1967,  1977,  1987,
    2022,  2056,  2068,  2074,  2094
};
#endif
...
#line 207 "1805063.y"
       ...
       if(error== 0){
                ...
                initial_code+="\tneg bx\n";
                initial_code+="\tnonneg_num:\n";
                initial_code+="\tmov ax,bx\n";
                initial_code+="\tmov bx, 10D\n";
                initial_code+="\txor cx, cx\n";
                initial_code+="\tcloop:\n";
                initial_code+="\txor dx, dx\n";
                initial_code+="\tdiv bx\n";
                initial_code+="\tpush dx\n";
                initial_code+="\tinc cx\n";
                initial_code+="\tcmp ax, 0\n";
                initial_code+="\tjne cloop\n";
                initial_code+="\tprint_loop:\n";
                initial_code+="\tpop dx\n";
                initial_code+="\tadd dx, '0'\n";
                initial_code+="\tmov ah, 2\n";
                initial_code+="\tint 21h\n";
                initial_code+="\tloop print_loop\n";
				...
				code_write((yyval.vectorsymbol)->at(0)->getCode());
				code_write("END main\n");
...
#line 479 "1805063.y"
        ...
        for (int i = 0; i < (yyvsp[-1].vectorsymbol)->size(); i++){      // Insert parameter into table
            if((yyvsp[-1].vectorsymbol)->at(i)->gettype() == "ID"){
                ...
                string symbol = (yyvsp[-1].vectorsymbol)->at(i)->getname() + "_" + table->getcurid();
                (table->lookup( (yyvsp[-1].vectorsymbol)->at(i)->getname()))->setSymbol(makeScopeID(symbol));
            }
        }
...
#line 869 "1805063.y"
        ...
        //assembly code
        string symbol = (yyvsp[0].symbol)->getname() + "_" + table->getcurid();
        (table->lookup((yyvsp[0].symbol)->getname()))->setSymbol(makeScopeID(symbol));
        data_list.push_back({makeScopeID(symbol),"0"});
    }
#line 2328 "y.tab.c"
    break;
...
#line 901 "1805063.y"
        ...
        //assembly code
			string symbol = (yyvsp[-3].symbol)->getname() + "_" + table->getcurid();
			(table->lookup((yyvsp[-3].symbol)->getname()))->setSymbol(makeScopeID(symbol));
...
#line 1255 "1805063.y"
        ...
        //assembly code
		string segment_code = "";
		segment_code += "\tmov ax, " + (table->lookup((yyvsp[-2].symbol)->getname()))->getSymbol() + "\n";
...
#line 1335 "1805063.y"
        ...
        //assembly code
		(yyval.vectorsymbol)->at(0)->setSymbol((table->lookup( (yyvsp[0].symbol)->getname()))->getSymbol());
...
#line 1372 "1805063.y"
        ...
        //assembly code
		(yyval.vectorsymbol)->at(0)->setSymbol((table->lookup((yyvsp[-3].symbol)->getname()))->getSymbol());
...
#line 1425 "1805063.y"
        ...
        //assembly code
        string whichvariable = (table->lookup((yyvsp[-2].vectorsymbol)->at(0)->getname()))->getwhichvar();
        string segment_code = "";
        string temp;
           ...
           // errorout("Error: Undeclared function "+ $1->getname() + "\n",line_count);
			...
			for(int i = 0; i < (yyvsp[-1].vectorsymbol)->size(); i++){
				if((yyvsp[-1].vectorsymbol)->at(i)->gettype() == "ID"){
                    ...
                    //assembly code
					pushed_var.push_back((table->lookup( (yyvsp[-1].vectorsymbol)->at(i)->getname()))->getSymbol());
					segment_code += "\tpush " + (table->lookup( (yyvsp[-1].vectorsymbol)->at(i)->getname()))->getSymbol() + "\n";
				}
			...
			for(int i = 0; i < (yyvsp[-1].vectorsymbol)->size(); i++){
				if((yyvsp[-1].vectorsymbol)->at(i)->gettype() == "ID"){
					segment_code += "\tmov ax, " + (table->lookup((yyvsp[-1].vectorsymbol)->at(i)->getname()))->getSymbol() + "\n";
...
#line 1987 "1805063.y"
		...
		string whichvariable = (table->lookup((yyvsp[-1].vectorsymbol)->at(0)->getname()))->getwhichvar();
